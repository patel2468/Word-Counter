    ...
    '<table class="table search_result_table">' +
      '<tr>' +
        '<td class="search_result_desc">' +
          ...
          '</div>' +          
        '</td>' +
        '<% if (searchResult.thumbnail && !searchResult.thumbnail.includes("DefaultImage.png")) { %>' +
          '<td class="search_result_additional_right thumb">' +
            '<a class="thumbnail_container" href="<%= resolveProductHelpDir(searchResult.product)%>/<%= searchResult.path %><%= appendSearchHighlight() %>" style="background-image:url(../<%=searchResult.thumbnail %>)"></a>' +
          '</td>' +
        '<% } %>' +
